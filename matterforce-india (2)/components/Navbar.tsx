...
import { ShoppingCart, Cpu, Store, User, Hexagon, Clapperboard, Wrench, Building } from 'lucide-react';
import { ViewState, CartItem, UserProfile } from '../types';
...
const Navbar: React.FC<NavbarProps> = ({ currentView, setView, cart, user }) => {
    ...
    <nav className="sticky top-0 z-50 w-full border-b border-matter-border bg-matter-dark/80 backdrop-blur-md">
      <div className="mx-auto flex max-w-7xl items-center justify-between px-4 py-4 sm:px-6 lg:px-8">
        ...
        <div 
          ...
          onClick={() => setView(isEngineer ? 'PROFILE' : 'HOME')}
        >
          ...
          <div className="flex flex-col">
            ...
            <span className="text-[10px] font-medium uppercase tracking-widest text-matter-orange/80">
                {isEngineer ? 'Service Lab' : isInstitution ? 'Enterprise' : 'Hardware Intelligence'}
            </span>
          </div>
        </div>
        ...
        <div className="hidden md:flex items-center space-x-8">
          ...
          {isEngineer && (
              <button 
                ...
                className={`flex items-center gap-2 text-sm font-medium transition-all duration-200 uppercase tracking-wide
                  ${currentView === 'PROFILE' ? 'text-purple-400 glow-text' : 'text-slate-400 hover:text-white'}`}
              >
          ...
          {isInstitution && (
              <button 
                onClick={() => setView('PROFILE')}
                className={`flex items-center gap-2 text-sm font-medium transition-all duration-200 uppercase tracking-wide
                  ${currentView === 'PROFILE' ? 'text-blue-400 glow-text' : 'text-slate-400 hover:text-white'}`}
              >
        ...
        <div className="flex items-center gap-4">
          ...
          <button 
            onClick={() => setView('PROFILE')}
            className={`p-2 transition-colors ${currentView === 'PROFILE' ? (isEngineer ? 'text-purple-400' : isInstitution ? 'text-blue-400' : 'text-matter-orange') : 'text-slate-400 hover:text-white'}`}
          >
