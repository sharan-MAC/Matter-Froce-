...
const Marketplace: React.FC<MarketplaceProps> = ({ addToCart }) => {
  ...
  const SYNONYMS: Record<string, string[]> = {
    'brain': ['microcontroller', 'computing', 'arduino', 'raspberry', 'esp32', 'plc', 'mcu', 'processor'],
    'muscle': ['motor', 'servo', 'actuator', 'stepper'],
    'eye': ['sensor', 'lidar', 'ultrasonic', 'dht22', 'vision', 'camera'],
    'power': ['battery', 'li-po', 'power', 'voltage', 'supply'],
    'juice': ['battery', 'li-po', 'power', 'voltage', 'supply'],
    'skeleton': ['materials', 'extrusion', 'carbon fiber', 'frame', 'chassis'],
    'body': ['materials', 'extrusion', 'carbon fiber', 'frame', 'chassis'],
    'wire': ['jumper', 'cable', 'breadboard', 'prototyping', 'connection'],
    'board': ['breadboard', 'arduino', 'raspberry', 'pcb'],
    'heart': ['power', 'battery', 'motor']
  };

  const filteredProducts = MARKETPLACE_DATA.filter(product => {
    const searchLower = searchTerm.toLowerCase().trim();
    
    let searchTermsToMatch = [searchLower];
    if (searchLower) {
      Object.entries(SYNONYMS).forEach(([key, values]) => {
        if (searchLower.includes(key) || key === searchLower) {
          searchTermsToMatch = [...searchTermsToMatch, ...values];
        }
      });
    }

    const searchableText = `${product.name} ${product.description} ${product.category} ${product.specs.join(' ')}`.toLowerCase();
    
    const matchesSearch = !searchLower || searchTermsToMatch.some(term => searchableText.includes(term));
