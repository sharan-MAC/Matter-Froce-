...
const AIErrorDisplay: React.FC<{ error: string; onDismiss: () => void; onRetry: () => void }> = ({ error, onDismiss, onRetry }) => (
          ...
          <div className="mt-12 animate-in fade-in slide-in-from-bottom-10 duration-700">
            ...
            <div className="grid grid-cols-1 lg:grid-cols-3 gap-8 mb-12">
              ...
              <div className="relative rounded-xl overflow-hidden bg-black border border-matter-border aspect-square lg:aspect-auto flex flex-col group perspective-1000">
                 ...
                 <div className={`absolute inset-0 bg-[radial-gradient(ellipse_at_center,_var(--tw-gradient-stops))] ${isIndustrial ? 'from-matter-orange/20' : 'from-matter-blue/20'} to-transparent opacity-50`}></div>
                 ...
                 <div className="relative w-full h-full flex items-center justify-center p-8 transition-transform duration-700 ease-out group-hover:scale-105 group-hover:rotate-1">
                    ...
                    <div className="relative w-full h-full max-h-[400px] max-w-[400px]">
                        ...
                        <div className={`absolute top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 w-[120%] h-[120%] border ${isIndustrial ? 'border-matter-orange/20' : 'border-matter-blue/20'} rounded-full animate-spin-slow`}></div>
                        <div className={`absolute top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 w-[90%] h-[90%] border border-dashed ${isIndustrial ? 'border-matter-orange/30' : 'border-matter-blue/30'} rounded-full animate-spin-reverse-slow`}></div>

                        <img 
                            src={`https://picsum.photos/seed/${result.productName.replace(/\s/g,'')}/600/600`}
                            alt="Blueprint"
                            className={`w-full h-full object-contain mix-blend-screen ${isIndustrial ? 'drop-shadow-[0_0_15px_rgba(249,115,22,0.5)]' : 'drop-shadow-[0_0_15px_rgba(59,130,246,0.5)]'} opacity-90`}
                        />
                        
                        {/* Scanning Laser */}
                        <div className={`absolute top-0 left-0 w-full h-0.5 ${bgColor} ${isIndustrial ? 'shadow-[0_0_20px_rgba(249,115,22,0.8)]' : 'shadow-[0_0_20px_rgba(59,130,246,0.8)]'} animate-scan z-20`}></div>
